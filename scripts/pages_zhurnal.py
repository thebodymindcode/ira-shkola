...
ISTOKI = {
 'nechist': [
  ...
  ('Где так говорили', 'Приметы держались не по всей стране одинаково. Где обычай известен только в части губерний, это сказано прямо в тексте.'),
  ('Где записи расходятся', 'Собиратели слышали разное даже в соседних деревнях. Спорные места оставлены спорными, а не сведены к одной удобной версии.'),
  ('Чего в источниках нет', 'Всё, чего в записях не нашлось, в текст не попало. Красивые подробности из современных пересказов сюда не идут.'),
 ],
 'oberegi': [
  ...
  ('Где так делали', 'Домашний обычай редко был общим для всей страны. Где он известен по части губерний, это названо в тексте.'),
  ('Где записи расходятся', 'В одной деревне клали под порог, в соседней на окно. Разночтения оставлены как есть, без сведения к одному правилу.'),
  ('Чего в источниках нет', 'Поздние толкования и советы из современных подборок в разбор не идут. Здесь только то, что записано у собирателей.'),
 ],
}
...
FINALY_STATEJ = [
 ...
 ('Рядом стоит соседний разбор', 'Существа народной веры держались парами и соседством, и половина примет понятна только вместе.',
  ('nechist/', 'Вся нечистая сила'), 'Смотреть соседей',
  ('Имя собирателя', 'У каждой приметы назван тот, кто её записал, и губерния.')),
 ...
 ('Спорное место в записях', 'У этой темы собиратели расходятся, и на курсе это разбирают отдельно: чья запись старше и почему версии разошлись.',
  ('kursy/besy/', 'Как это проходят на курсе'), 'Спросить Ирину',
  ('Две версии', 'Столько бывает у одной приметы в соседних губерниях.')),
]
...
<details class="toc" open><summary>Что внутри</summary><ol>{''.join(toc)}</ol></details>
{secs}
...
{finalny('Что выходит дальше',
         'Новые тексты выходят в канале школы, и там же их обсуждают в комментариях.',
...
SIDE_RAZDELA = {
 'oberegi': 'обрядов дома разобрано здесь, у каждого названы запись и губерния.',
 'nechist': 'существ народной веры собрано в разделе, от домовых до заложных покойников.',
}
...
def razdel(kind, articles):
    ...
    if kind == 'oberegi':
        ...
        vvod = [
            ...
            'Запреты выросли отсюда же. Сегодня они выглядят суеверием, а в деревне были обычным порядком дня.',
        ...
        vvod = [
            ...
            'Обереги поэтому разные. Того, кто живёт в доме, задабривали угощением и поклоном. '
