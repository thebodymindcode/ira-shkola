...
.kadr.art-kadr{border-radius:20px}
...
.split .ph img{position:absolute;inset:0;width:100%;height:100%;object-fit:contain;
  display:block;z-index:2}
.ph>i.fon{position:absolute;inset:-8%;background-size:cover;background-position:50% 40%;
  filter:blur(38px) brightness(.5) saturate(.85);transform:scale(1.18);z-index:1}
.ph{background:#0E0C11}
.kadr .ph img{position:relative;z-index:2}
...
/* ---------- вопросник ---------- */
.kviz{background:var(--sloy);border:1px solid var(--line);border-radius:20px;padding:34px}
.kviz-shag{display:flex;align-items:center;gap:18px;margin-bottom:22px}
...
.kviz-polosa{flex:1;height:3px;background:rgba(232,226,217,.12);border-radius:2px;overflow:hidden}
.kviz-polosa i{display:block;height:100%;background:var(--zoloto);transition:width .3s}
.kviz-vopros{font-size:clamp(23px,3vw,32px);margin:0 0 22px}
...
.kviz-otvet{text-align:left;font:inherit;font-size:16.5px;color:#DCD5CA;background:var(--sloy2);
  ...
  transition:border-color .16s,transform .12s;min-height:52px}
.kviz-otvet:active{transform:scale(.995)}
@media(hover:hover){.kviz-otvet:hover{border-color:rgba(201,162,39,.55);color:#F0E9DE}}
.kviz-itog{display:grid;grid-template-columns:180px minmax(0,1fr);gap:32px;align-items:center;
  margin-top:8px}
.kviz-itog svg.ark{width:100%;height:auto;display:block}
.kviz-itog h3{margin:2px 0 12px;font-size:30px}
.kviz-itog p{color:var(--tihiy);font-size:16.5px;margin:0 0 12px}
.kviz-klyuchi{color:var(--zoloto-svet)!important;font-size:15px!important}
@media(max-width:640px){.kviz{padding:22px 18px}
  .kviz-itog{grid-template-columns:1fr!important;gap:20px;justify-items:center;text-align:center}
  .kviz-itog .kviz-karta{max-width:170px}
  .kviz-itog .knopki{justify-content:center}}
...
@media(max-width:1000px){
  ...
  .split .ph{height:auto!important;aspect-ratio:3/4;min-height:0;position:relative;
