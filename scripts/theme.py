...
.hero{position:relative;padding:0;border:0;overflow:hidden}
.hero .fon{position:absolute;inset:0;overflow:hidden}
.hero .fon img{width:100%;height:100%;object-fit:cover;object-position:50% 40%;opacity:.16;
  filter:blur(3px)}
.hero .fon::after{content:"";position:absolute;inset:0;
  background:linear-gradient(180deg,rgba(14,12,17,.72) 0%,rgba(14,12,17,.86) 55%,var(--noch) 100%)}
...
@media(max-width:1000px){
  .hero .ryad{grid-template-columns:1fr!important;gap:30px}
  .hero .portret{aspect-ratio:16/10;max-height:340px}
}
