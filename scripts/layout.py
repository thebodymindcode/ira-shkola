...
(function(){
 ...
 var celi=document.querySelectorAll('.side .cifra, .final .fside b, .nail b');
 var nabl=new IntersectionObserver(function(z){
  z.forEach(function(e){
   ...
   var t0=null, dlit=900;
   function shag(t){ if(!t0) t0=t;
    var k=Math.min(1,(t-t0)/dlit), e2=1-Math.pow(1-k,3);
    el.textContent=do_+Math.round(chislo*e2).toLocaleString('ru-RU')+posle;
    if(k<1) requestAnimationFrame(shag);
   }
   el.textContent=do_+'0'+posle; requestAnimationFrame(shag);
  });
 },{threshold:0.6});
